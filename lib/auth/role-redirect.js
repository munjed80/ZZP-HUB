import { safeNextUrl } from './safe-next.js';
 ...
 * - Uses safeNextUrl to prevent redirect loops and validate paths
 */
export function resolveLoginRedirect({ role, selectedType, requestedRedirect }) {
  ...
  // Use safeNextUrl to validate and sanitize the requested redirect
  // This prevents nested next parameters and redirect loops
  let target = safeNextUrl(requestedRedirect, typeDefault);
  ...
  // Ensure accountants never land on dashboard variants
  if (isAccountant && target.startsWith("/dashboard")) {
    target = "/accountant-portal";
  }
