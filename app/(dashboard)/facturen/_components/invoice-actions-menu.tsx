...
export function InvoiceActionsMenu({ pdfInvoice, invoiceId, editHref, shareLink, isOpen, onOpenChange }: Props) {
  ...
  const menuItemClasses =
    "h-10 w-full justify-start gap-2 rounded-xl border border-btn-border bg-btn text-slate-900 dark:text-slate-50 hover:bg-btn-hover hover:text-slate-900 dark:hover:text-slate-50 [&>svg]:text-slate-700 dark:[&>svg]:text-slate-200 disabled:text-slate-400 disabled:opacity-60";
  const menuContent = (
      ...
      <div className="mt-2 flex flex-col gap-1.5">
        {shareLink ? (
          <Link href={shareLink} className={buttonVariants("ghost", menuItemClasses)} onClick={() => setMenuOpen(false)}>
            <Eye className="h-4 w-4 text-slate-700 dark:text-slate-200" aria-hidden />
            Bekijk factuur
          </Link>
        ) : null}
        {editHref ? (
          <Link href={editHref} className={buttonVariants("ghost", menuItemClasses)} onClick={() => setMenuOpen(false)}>
            <Edit3 className="h-4 w-4 text-slate-700 dark:text-slate-200" aria-hidden />
            Bewerk factuur
          </Link>
        ) : null}
        <InvoicePdfDownloadButton
          ...
          variant="ghost"
          icon={<FileDown className="h-4 w-4 text-slate-700 dark:text-slate-200" aria-hidden />}
        />
        <button type="button" onClick={handleShare} className={cn(buttonVariants("ghost", menuItemClasses))}>
          <Share2 className="h-4 w-4 text-slate-700 dark:text-slate-200" aria-hidden />
          Deel
        </button>
        <button
          type="button"
