  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-bold text-foreground">Factuurweergave</h1>
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="w-1/2 text-right">
              {bedrijf.logoUrl ? (
                ...
                <p className="text-2xl font-bold tracking-tight text-foreground">{bedrijf.companyName}</p>
              )}
            </div>
          </div>
        </CardHeader>
        ...
        <CardContent className="space-y-6">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="rounded-lg border border-border p-4">
              <p className="text-sm font-semibold text-foreground">Verzender</p>
              <p className="text-sm text-muted-foreground">{bedrijf.companyName}</p>
              <p className="text-sm text-muted-foreground">
                {bedrijf.address}, {bedrijf.postalCode} {bedrijf.city}
              </p>
              <p className="text-sm text-muted-foreground">KVK: {bedrijf.kvkNumber}</p>
              <p className="text-sm text-muted-foreground">BTW: {bedrijf.btwNumber}</p>
              <p className="text-sm text-muted-foreground">IBAN: {bedrijf.iban} ({bedrijf.bankName})</p>
            </div>

            <div className="rounded-lg border border-border p-4">
              <p className="text-sm font-semibold text-foreground">Ontvanger</p>
              <p className="text-sm text-muted-foreground">{klant.naam}</p>
              <p className="text-sm text-muted-foreground">{klant.adres}</p>
              <p className="text-sm text-muted-foreground">
                {klant.postcode} {klant.plaats}
              </p>
              <p className="text-sm text-muted-foreground">BTW-id: {klant.btw}</p>
            </div>
          </div>
            ...
            <div className="divide-y divide-border">
              {regels.map((regel) => (
                <div
                  ...
                  className="grid grid-cols-5 px-4 py-3 text-sm text-foreground"
                >
          ...
          <div className="flex flex-col items-end gap-1 text-sm text-foreground">
            ...
            <div className="flex w-full max-w-md items-center justify-between">
              ...
              <span className="font-semibold text-foreground">Automatisch berekend</span>
            </div>
            <div className="flex w-full max-w-md items-center justify-between text-base font-bold">
              <span>Totaal te betalen</span>
              <span>{formatBedrag(totaal)}</span>
            </div>
            <p className="mt-2 text-xs text-muted-foreground">Betaaltermijn: {bedrijf.paymentTerms}</p>
          </div>

          <div className="flex items-center justify-between rounded-lg bg-muted px-4 py-3 text-sm text-foreground">
