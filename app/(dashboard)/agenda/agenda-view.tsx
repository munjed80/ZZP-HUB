...
function AddEventModal({ open, onClose, onAdded }: AddEventModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex min-h-screen items-center justify-center bg-[rgb(var(--bg-main))]/70 px-4 py-6">
      <div className="w-full max-w-lg rounded-lg bg-card shadow-lg border border-border">
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-[rgb(var(--brand-primary))]">Nieuwe afspraak</p>
            <h2 className="text-lg font-bold text-foreground">Voeg een afspraak toe aan je agenda</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="rounded-full p-2 text-muted-foreground transition hover:bg-[rgb(var(--bg-surface))]"
            aria-label="Sluiten"
          >
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-6 py-5">
          <div className="space-y-2">
            <label className="text-sm font-medium text-foreground" htmlFor="event-title">
              ...
              <input
                ...
                className="w-full rounded-lg border border-border px-3 py-2 text-sm text-foreground shadow-inner focus:border-[rgb(var(--brand-primary))] focus:outline-none focus:ring-2 focus:ring-[rgb(var(--brand-primary))]/40"
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <textarea
                ...
                className="min-h-[72px] w-full rounded-lg border border-border bg-card pl-10 pr-3 py-2 text-sm text-foreground shadow-inner focus:border-[rgb(var(--brand-primary))] focus:outline-none focus:ring-2 focus:ring-[rgb(var(--brand-primary))]/40"
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-border bg-card pl-10 pr-3 py-2 text-sm text-foreground shadow-inner focus:border-[rgb(var(--brand-primary))] focus:outline-none focus:ring-2 focus:ring-[rgb(var(--brand-primary))]/40"
                />
              </div>
            </div>
            <div className="space-y-2">
          ...
          <div className="flex items-center justify-end gap-2 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="rounded-lg px-4 py-2 text-sm font-semibold text-muted-foreground transition hover:bg-[rgb(var(--bg-surface))]"
            >
              Annuleren
            </button>
            <button
              type="submit"
              disabled={isPending}
              className="inline-flex items-center gap-2 rounded-lg bg-[rgb(var(--brand-primary))] px-4 py-2 text-sm font-semibold text-[rgb(var(--brand-on-primary))] shadow-sm transition hover:bg-[rgb(var(--brand-primary-hover))] focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[rgb(var(--brand-primary))] disabled:cursor-not-allowed disabled:opacity-70"
            >
  ...
  return (
    <>
      <div className="space-y-8">
        ...
        <div className="grid gap-4 lg:grid-cols-[2fr_1fr]">
          <Card>
            <CardHeader className="pb-4">
              <div className="flex items-center justify-between">
                <div>
                   <CardTitle className="text-base font-medium text-foreground">Kalender</CardTitle>
             ...
             <CardContent className="space-y-4">
              ...
              <div className="hidden md:grid grid-cols-7 gap-2">
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        dagEvents.length > 0 
                          ? "border-[rgb(var(--brand-primary))] bg-primary/10" 
                     ...
                     <div className="flex items-center justify-between">
                       <span
                         className={cn(
                           ...
                           isVandaag(dag) 
                              ? "bg-[rgb(var(--brand-primary))] text-[rgb(var(--brand-on-primary))]" 
                              : "text-foreground",
                          )}
                      ...
                      <div className="mt-2 space-y-1.5">
                        {dagEvents.map((event) => (
                           <div
                             key={event.id}
                             className="rounded px-2 py-1.5 bg-card border border-[rgb(var(--brand-primary))]/50"
                           >
                ...
                {upcomingEvents.length === 0 ? (
                  ...
                  <div className="relative space-y-4 pl-5 before:absolute before:left-[6px] before:top-0 before:h-full before:w-[2px] before:bg-[rgb(var(--brand-primary))]">
                    {upcomingEvents.map((event) => (
                      ...
                      >
                        <span
                          aria-hidden="true"
                          className="absolute -left-[14px] top-4 inline-flex h-3.5 w-3.5 items-center justify-center rounded-full border border-card bg-[rgb(var(--brand-primary))] shadow-sm"
                        />
                        ...
                        <div className="mt-3 flex items-center gap-2 rounded-lg border border-border bg-[rgb(var(--bg-surface))] px-3 py-2 text-xs font-semibold text-foreground">
          ...
          <Card>
            <CardHeader className="pb-4">
              <div className="flex items-center justify-between">
                 <CardTitle className="text-base font-medium text-foreground">Aankomende afspraken</CardTitle>
      ...
      <button
        type="button"
        onClick={() => setShowAddModal(true)}
        className="fixed bottom-6 right-6 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-[rgb(var(--brand-primary))] text-[rgb(var(--brand-on-primary))] border border-[rgb(var(--brand-primary-active))] shadow-lg transition hover:bg-[rgb(var(--brand-primary-hover))] focus:outline-none focus:ring-4 focus:ring-[rgb(var(--brand-primary))]/30 md:hidden"
